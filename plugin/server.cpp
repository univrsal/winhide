...
bool server::connect_client()
{
    ...
    if (m_sockets) {
        if (netlib_tcp_add_socket(m_sockets, m_server_socket) < 0 || netlib_tcp_add_socket(m_sockets, m_client_socket) < 0) {
            result = false;
        } else {
        }
    } else {
...
{
    return netlib_read_uint16(b, &r->x) && netlib_read_uint16(b, &r->y) && netlib_read_uint16(b, &r->w) && netlib_read_uint16(b, &r->h);
}
